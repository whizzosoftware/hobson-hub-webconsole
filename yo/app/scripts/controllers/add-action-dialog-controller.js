        ...
        function($scope, PluginsService, DevicesService, ActionsService, DialogContextService) {
                ...
                ActionsService.getAction($scope.actions[$scope.actionId]).then(function(response) {
                    ...
                    for (var key in response.meta) {
                        ...
                        if (key === 'pluginId') {
                            ...
                            formOptions.label = 'Plugin'; // change Plugin ID -> Plugin since we're giving the user a dropdown
                            formOptions.options = [];
                            pluginFormOptions = formOptions.options;
                            hadPluginKey = true;
                    ...
                    // if there was a pluginId key, retrieve the list of plugins
                    if (hadPluginKey) {
                        PluginsService.getPlugins(false, false).then(function(result) {
                            $scope.plugins = {};
                            for (var i in result) {
                                var plugin = result[i];
                                if (plugin.type === 'PLUGIN' && plugin.status.status === 'RUNNING') {
                                    pluginFormOptions.push({name: plugin.name, value: plugin.id});
                                    $scope.plugins[plugin.id] = plugin;
                                }
                            }
                        });
                    }
                    ...
                    if (hadDeviceKey) {
                        $scope.$watch('formData.pluginId', function (pluginId) {
                            if (pluginId) {
                                DevicesService.getDevicesForPlugin($scope.plugins[pluginId]).then(function (results) {
                                    $scope.devices = {};
            ...
            $scope.add = function(form) {
                if (form.$valid) {
                    ...
                    DialogContextService.getParams().trigger.actions.push(action);
